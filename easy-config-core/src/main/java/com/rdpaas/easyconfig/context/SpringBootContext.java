...
public class SpringBootContext implements ApplicationContextAware {
    ...
    public void setApplicationContext(ApplicationContext ac) throws BeansException {
        ...
        try {
            ...
            if (PropUtil.isWebProp(filePath))
                ...
                if (!file.isDirectory()) // 谁让java就是nb，只能监听目录
                    dir = file.getParent();
            ...
            log.error("init refresh bean error", e);
        }
    }
            ...
            if (bd instanceof AnnotatedBeanDefinition) {
                ...
                if (factoryMethodMeta != null) {
                    ...
                    if (at.isAnnotated(REFRESH_SCOPE_ANNOTATION_NAME)) {
                        ...
                        refreshScopeBeanInvokorMap.put(clazz, new SpringAnnotatedRefreshScopeBeanInvoker(false, defaultListableBeanFactory, beanName, (AnnotatedBeanDefinition) bd, clazz, null));
                    }
                }
    ...
    public void refreshConfig(Map<String, Object> props) throws InvocationTargetException, IllegalAccessException {
        ...
        for (Map.Entry<Class<?>, SpringAnnotatedRefreshScopeBeanInvoker> entry : refreshScopeBeanInvokorMap.entrySet()) {
            SpringAnnotatedRefreshScopeBeanInvoker invoker = entry.getValue();
            boolean isMethod = invoker.isMethod();

            // 判断执行器是不是代表的一个@Bean修饰的方法
            if (isMethod)
                invoker.refreshPropsIntoBean(props); // 使用执行器将属性刷新到@Bean修饰的方法产生的对象中,使其支持@ConfigurationProperties注解
            else
                invoker.refreshPropsIntoField(props); //使用执行器将属性刷新到对象中
